...
/**
 * What a person may state about a plugin, and how it is checked.
 *
 * Split out of `Submissions.js`, which had passed the point where it had one
 * reason to change. This is the half that changes when a field is added: the
 * `SUBMITTABLE` table with its vocabulary composition, and the
 * `validate` / `valueTerm` pair that read it. The queueing, reviewing and
 * writing — the moderation flow — stayed in `Submissions.js`, which
 * re-exports everything here so no caller moves.
 */
