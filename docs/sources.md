...
| GitHub API | [Acceptable Use Policies](https://docs.github.com/en/site-policy/acceptable-use-policies/github-acceptable-use-policies): "Scraping does not refer to the collection of information through our API." Prohibits using collected information "for spamming purposes... or selling personal information". | **Usable via the API only.** Never scrape GitHub HTML. Respect rate limits, authenticate. See the personal-data rule below. Implemented in `GitHubClient`: a token with no scopes, conditional requests so a re-harvest costs almost no quota, waits for the published reset rather than retrying around a limit, and no call to `/commits`, `/contributors` or `/users` — the endpoints that carry email addresses. |
| Repositories harvested from GitHub | Per repository, read from the API's `license.spdx_id` at harvest time | **One graph per repository, carrying that repository's licence.** A licence the graph registry does not recognise, or none at all, becomes `unknown`, which is flagged not redistributable — so an unlicensed repository is discovered and listed but not harvested until a person decides otherwise. Silence is not permission. |
